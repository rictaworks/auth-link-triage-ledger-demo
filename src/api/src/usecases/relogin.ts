...
export async function getReloginPlan(
  ...
  const existingOrigins = await repo.listCaseOrigins(sessionId, triage.caseId);
  if (!existingOrigins.some((o) => o.service_id === originId)) {
  ...
  const currentCase = await repo.findCase(sessionId, triage.caseId);
  if (currentCase && currentCase.state !== "resolved" && currentCase.state !== "origin_selected") {
    await repo.updateCaseState(sessionId, triage.caseId, "origin_selected", null);
  }

  const { graph, links, observations, statusMap, windowStart, windowEnd } = await buildGraphAndStatus(repo, sessionId, now);
  const onset = resolveCaseOnset(triage.caseId, observations, windowStart, windowEnd);
